...
same season. The algorithm uses histograms with minimum and maximum
...
the CDO option –timestat_date \<first\|middle\|last\>. Be careful about
the first and the last output timestep, they may be incorrect values if
the seasons have incomplete timesteps. For every adjacent sequence t_1,
...,t_n of timesteps of the same season it is: o(t,x) = pth percentile
{i(t',x), t1 \< t' \<= tn}
