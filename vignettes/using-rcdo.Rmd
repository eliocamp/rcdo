...
The rcdo package translates R functions into CDO commands. Each CDO operator has an equivalent rcdo function, which prefixed with `cdo_`. So, if you want to use the `monmean` CDO operator to resample a time series into monthly values, you would use the `cdo_monmean()` function.

**Important**\
The whole rcdo package and its documentation is built automatically from the CDO source. This comes with some limitations.

1.  CDO operators are documented in "families" where all parameters are documented together. Currently there is no way for the build process to correctly attribute each parameter to the correct operator. This unfortunately means that rcdo functions have every argument from a particular family. For example, the functions `cdo_selindexbox()` and `cdo_sellonlatbox()` both have `lon1` in their signature.
2.  The conversion from rcdo function arguments to CDO command parameters is pretty dumb. Argument names re only for the user convenience and are not really used: function arguments are converted to CDO parameters simply in the order they are defined in the function signature. Neither are argument checked for validity. This means that `cdo_sellonlatbox(file, lon1 = 0)` will not return an error even though the operator is missing other necessary arguments. Also `cdo_sellonlatbox(file, idx1 = 0)` is identical to `cdo_sellonlatbox(file, lon1 = 0)`.
3.  Some CDO operators need named parameters. rcdo currently doesn't know how to deal with that, so you need to pass the names yourself. So `cdo_select(file, name = "temperature")` doesn't work, you need to do `cdo_select(file, name = "name=temperature")`, which is equivalent to `cdo_select(file, code = "name=temperature")` due to the previously described limitation.
4.  Some parameters need to be quoted. For example, the expression in the `expr` operator needs to be surrounded by quotes. Again, rcdo doesn't know this so you must "double quote" the argument yourself; i.e. `cdo_expr(file, "'t_celcius=t-273.15'")`.
5.  Some documentation formatting might be incorrect. If you spot some part of the documentation that didn't survive the conversion, [open an issue](https://github.com/eliocamp/rcdo/issues)!
...
#> Warning: Using CDO version 2.4.3 which is different from the version supported by this version of rcdo (2.5.1).
...
#> [1] "/tmp/Rtmp9eAVmx/filee0a5e23a06782"
#> attr(,"mtime")
#> [1] "2025-05-14 10:19:00 AEST"
#> attr(,"size")
#> [1] 1580009
```
...
sh_file <- sh |> 
  cdo_execute()
sh_file
#> [1] "/tmp/Rtmp9eAVmx/filee0a5e5e7215ed"
#> attr(,"ephemeral")
#> attr(,"ephemeral")[[1]]
...
Some operators take one file and return an undetermined number of files. `splitmon` will return one file per month. Unfortunately rcdo cannot return the list of files created. The returned string is the base suffix shared by all files.
...
mon_split <- sh_500 |> 
  ...
  cdo_execute()
mon_split
#> [1] "/tmp/Rtmp9eAVmx/filee0a5e796a57e9"
#> attr(,"ephemeral")
#> attr(,"ephemeral")[[1]]
#> File will be deleted when garbage collected
```
...
mon_split <- paste0(mon_split, "*") |> 
  Sys.glob()
mon_split
#>  [1] "/tmp/Rtmp9eAVmx/filee0a5e796a57e901.nc" "/tmp/Rtmp9eAVmx/filee0a5e796a57e902.nc"
#>  [3] "/tmp/Rtmp9eAVmx/filee0a5e796a57e903.nc" "/tmp/Rtmp9eAVmx/filee0a5e796a57e904.nc"
#>  [5] "/tmp/Rtmp9eAVmx/filee0a5e796a57e905.nc" "/tmp/Rtmp9eAVmx/filee0a5e796a57e906.nc"
#>  [7] "/tmp/Rtmp9eAVmx/filee0a5e796a57e907.nc" "/tmp/Rtmp9eAVmx/filee0a5e796a57e908.nc"
#>  [9] "/tmp/Rtmp9eAVmx/filee0a5e796a57e909.nc" "/tmp/Rtmp9eAVmx/filee0a5e796a57e910.nc"
#> [11] "/tmp/Rtmp9eAVmx/filee0a5e796a57e911.nc" "/tmp/Rtmp9eAVmx/filee0a5e796a57e912.nc"
```
...
#> [[1]]
#> CDO command:
#>    /home/user1/.local/share/R/rcdo/cdo-2.5.1/bin/cdo deltat [ '/tmp/Rtmp9eAVmx/filee0a5e796a57e901.nc' ] {{output}} 
#> 
#> [[2]]
#> CDO command:
#>    /home/user1/.local/share/R/rcdo/cdo-2.5.1/bin/cdo deltat [ '/tmp/Rtmp9eAVmx/filee0a5e796a57e902.nc' ] {{output}} 
#> 
#> [[3]]
#> CDO command:
#>    /home/user1/.local/share/R/rcdo/cdo-2.5.1/bin/cdo deltat [ '/tmp/Rtmp9eAVmx/filee0a5e796a57e903.nc' ] {{output}} 
#> 
#> [[4]]
#> CDO command:
#>    /home/user1/.local/share/R/rcdo/cdo-2.5.1/bin/cdo deltat [ '/tmp/Rtmp9eAVmx/filee0a5e796a57e904.nc' ] {{output}} 
#> 
#> [[5]]
#> CDO command:
#>    /home/user1/.local/share/R/rcdo/cdo-2.5.1/bin/cdo deltat [ '/tmp/Rtmp9eAVmx/filee0a5e796a57e905.nc' ] {{output}} 
#> 
#> [[6]]
#> CDO command:
#>    /home/user1/.local/share/R/rcdo/cdo-2.5.1/bin/cdo deltat [ '/tmp/Rtmp9eAVmx/filee0a5e796a57e906.nc' ] {{output}} 
#> 
#> [[7]]
#> CDO command:
#>    /home/user1/.local/share/R/rcdo/cdo-2.5.1/bin/cdo deltat [ '/tmp/Rtmp9eAVmx/filee0a5e796a57e907.nc' ] {{output}} 
#> 
#> [[8]]
#> CDO command:
#>    /home/user1/.local/share/R/rcdo/cdo-2.5.1/bin/cdo deltat [ '/tmp/Rtmp9eAVmx/filee0a5e796a57e908.nc' ] {{output}} 
#> 
#> [[9]]
#> CDO command:
#>    /home/user1/.local/share/R/rcdo/cdo-2.5.1/bin/cdo deltat [ '/tmp/Rtmp9eAVmx/filee0a5e796a57e909.nc' ] {{output}} 
#> 
#> [[10]]
#> CDO command:
#>    /home/user1/.local/share/R/rcdo/cdo-2.5.1/bin/cdo deltat [ '/tmp/Rtmp9eAVmx/filee0a5e796a57e910.nc' ] {{output}} 
#> 
#> [[11]]
#> CDO command:
#>    /home/user1/.local/share/R/rcdo/cdo-2.5.1/bin/cdo deltat [ '/tmp/Rtmp9eAVmx/filee0a5e796a57e911.nc' ] {{output}} 
#> 
#> [[12]]
#> CDO command:
#>    /home/user1/.local/share/R/rcdo/cdo-2.5.1/bin/cdo deltat [ '/tmp/Rtmp9eAVmx/filee0a5e796a57e912.nc' ] {{output}}
```
...
mon_split |> 
  ...
  cdo_execute_list()
#> [[1]]
#> [1] "/tmp/Rtmp9eAVmx/filee0a5e361f6a37"
#> attr(,"ephemeral")
#> attr(,"ephemeral")[[1]]
#> File will be deleted when garbage collected
#> 
...
#> 
#> [[2]]
#> [1] "/tmp/Rtmp9eAVmx/filee0a5e2eda7fb4"
#> attr(,"ephemeral")
#> attr(,"ephemeral")[[1]]
#> File will be deleted when garbage collected
#> 
...
#> 
#> [[3]]
#> [1] "/tmp/Rtmp9eAVmx/filee0a5e183416f2"
#> attr(,"ephemeral")
#> attr(,"ephemeral")[[1]]
#> File will be deleted when garbage collected
#> 
...
#> 
#> [[4]]
#> [1] "/tmp/Rtmp9eAVmx/filee0a5e6231c7bb"
#> attr(,"ephemeral")
#> attr(,"ephemeral")[[1]]
#> File will be deleted when garbage collected
#> 
...
#> 
#> [[5]]
#> [1] "/tmp/Rtmp9eAVmx/filee0a5e42710772"
#> attr(,"ephemeral")
#> attr(,"ephemeral")[[1]]
#> File will be deleted when garbage collected
#> 
...
#> 
#> [[6]]
#> [1] "/tmp/Rtmp9eAVmx/filee0a5e76a62cdf"
#> attr(,"ephemeral")
#> attr(,"ephemeral")[[1]]
#> File will be deleted when garbage collected
#> 
#> attr(,"mtime")
#> [1] "2025-05-14 10:19:01 AEST"
#> attr(,"size")
#> [1] 59545
#> 
#> [[7]]
#> [1] "/tmp/Rtmp9eAVmx/filee0a5e664af4e4"
#> attr(,"ephemeral")
#> attr(,"ephemeral")[[1]]
#> File will be deleted when garbage collected
#> 
...
#> 
#> [[8]]
#> [1] "/tmp/Rtmp9eAVmx/filee0a5e2e6c60de"
#> attr(,"ephemeral")
#> attr(,"ephemeral")[[1]]
#> File will be deleted when garbage collected
#> 
...
#> 
#> [[9]]
#> [1] "/tmp/Rtmp9eAVmx/filee0a5e2ab5ff3b"
#> attr(,"ephemeral")
#> attr(,"ephemeral")[[1]]
#> File will be deleted when garbage collected
#> 
...
#> 
#> [[10]]
#> [1] "/tmp/Rtmp9eAVmx/filee0a5e32528eec"
#> attr(,"ephemeral")
#> attr(,"ephemeral")[[1]]
#> File will be deleted when garbage collected
#> 
...
#> 
#> [[11]]
#> [1] "/tmp/Rtmp9eAVmx/filee0a5e78041089"
#> attr(,"ephemeral")
#> attr(,"ephemeral")[[1]]
#> File will be deleted when garbage collected
#> 
...
#> 
#> [[12]]
#> [1] "/tmp/Rtmp9eAVmx/filee0a5e596856ea"
#> attr(,"ephemeral")
#> attr(,"ephemeral")[[1]]
#> File will be deleted when garbage collected
#> 
...
#> [[1]]
#> [1] "/tmp/Rtmp9eAVmx/filee0a5e5936ef67"
#> attr(,"mtime")
#> [1] "2025-05-14 10:19:01 AEST"
#> attr(,"size")
#> [1] 59545
#> 
#> [[2]]
#> [1] "/tmp/Rtmp9eAVmx/filee0a5ef6fd9e3"
#> attr(,"mtime")
#> [1] "2025-05-14 10:19:01 AEST"
#> attr(,"size")
#> [1] 59545
#> 
#> [[3]]
#> [1] "/tmp/Rtmp9eAVmx/filee0a5e62e74885"
#> attr(,"mtime")
#> [1] "2025-05-14 10:19:01 AEST"
#> attr(,"size")
#> [1] 59545
#> 
#> [[4]]
#> [1] "/tmp/Rtmp9eAVmx/filee0a5e675ab51c"
#> attr(,"mtime")
#> [1] "2025-05-14 10:19:01 AEST"
#> attr(,"size")
#> [1] 59545
#> 
#> [[5]]
#> [1] "/tmp/Rtmp9eAVmx/filee0a5ec309ad5"
#> attr(,"mtime")
#> [1] "2025-05-14 10:19:01 AEST"
#> attr(,"size")
#> [1] 59545
#> 
#> [[6]]
#> [1] "/tmp/Rtmp9eAVmx/filee0a5e52a3c062"
#> attr(,"mtime")
#> [1] "2025-05-14 10:19:01 AEST"
#> attr(,"size")
#> [1] 59545
#> 
#> [[7]]
#> [1] "/tmp/Rtmp9eAVmx/filee0a5e6610b684"
#> attr(,"mtime")
#> [1] "2025-05-14 10:19:01 AEST"
#> attr(,"size")
#> [1] 59545
#> 
#> [[8]]
#> [1] "/tmp/Rtmp9eAVmx/filee0a5e4843926a"
#> attr(,"mtime")
#> [1] "2025-05-14 10:19:01 AEST"
#> attr(,"size")
#> [1] 59545
#> 
#> [[9]]
#> [1] "/tmp/Rtmp9eAVmx/filee0a5e4c0e184c"
#> attr(,"mtime")
#> [1] "2025-05-14 10:19:01 AEST"
#> attr(,"size")
#> [1] 59545
#> 
#> [[10]]
#> [1] "/tmp/Rtmp9eAVmx/filee0a5e1050d1a7"
#> attr(,"mtime")
#> [1] "2025-05-14 10:19:01 AEST"
#> attr(,"size")
#> [1] 59545
#> 
#> [[11]]
#> [1] "/tmp/Rtmp9eAVmx/filee0a5e42cda9f0"
#> attr(,"mtime")
#> [1] "2025-05-14 10:19:01 AEST"
#> attr(,"size")
#> [1] 59545
#> 
#> [[12]]
#> [1] "/tmp/Rtmp9eAVmx/filee0a5e7f499b34"
#> attr(,"mtime")
#> [1] "2025-05-14 10:19:01 AEST"
#> attr(,"size")
#> [1] 59545
...
mon_split |> 
  ...
  cdo_mergetime() |> 
  cdo_execute()
#> [1] "/tmp/Rtmp9eAVmx/filee0a5e2bc6127a"
#> attr(,"ephemeral")
#> attr(,"ephemeral")[[1]]
#> File will be deleted when garbage collected
#> 
